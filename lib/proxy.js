...
ReverseProxy.prototype.setupHttpsProxy = function (proxyParams, websocketsUpgrade, log, sslOpts) {
  ...
  if (sslOpts.http2) {
    srvConsturctor = sslOpts.serverModule || require('http2');
    sslOpts.allowHTTP1 = true;
  } else {
    srvConsturctor = sslOpts.serverModule || require('https');
  }
  ...
  const httpsServer = (this.httpsServer = srvConsturctor[sslOpts.http2 ? 'createSecureServer' : 'createServer'](ssl, (req, res) => {
    const src = this._getSource(req);
