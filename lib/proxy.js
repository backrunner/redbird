...
ReverseProxy.prototype.register = function (src, target, opts) {
  ...
  this.log && this.log.info(`Registered a new route - ${from} -> ${to}`);
  return this;
};
...
ReverseProxy.prototype.updateCertificates = async function (
  ...
  if (renew) {
    logger.warn('renew parameter is deprecated and is automatically detected by greenlock.');
  }

  if (renewWithin) {
    logger.warn('production parameter is deprecated.');
  }

  const siteCallback = (err, site) => {
    if (err) {
      if (err.message.contains('ENOENT: no such file or directory')) {
        this.log && this.log.warn(err, err.message);
      } else {
        this.log && this.log.error(err, 'Error getting LetsEncrypt certificates');
        return;
      }
    }
    if (!site || !site.pems || !site.pems.privkey || !site.pems.fullchain) {
      //
      // TODO: Try again, but we need an exponential back of to avoid getting banned.
      //
      this.log && this.log.info(`Could not get any certs for ${domain}.`);
      return;
    }
    const opts = {
  ...
  letsencrypt.getCertificates(domain, email, production, this.log, siteCallback);
};
...
ReverseProxy.prototype._getTarget = function (src, req, res) {
  ...
  return this.resolve(src, url, req).then((route) => {
    if (!route) {
      this.log && this.log.warn(`No valid route found for given source - ${src} -> ${url}`);
      return;
    }
    ...
    this.log &&
      this.log.debug(`Proxying ${src + url} to ${path.posix.join(target.host, req.url)}.`);
...
function redirectToHttps(req, res, target, ssl, log) {
  ...
  log && log.debug(`Redirecting ${path.posix.join(req.headers.host, req.url)} to ${url}.`);
  //
