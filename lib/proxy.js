...
ReverseProxy.prototype.setupLetsencrypt = function (log, opts) {
  ...
  log && log.debug('Chanllenge resolver added.');
};
...
ReverseProxy.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade, log, sslOpts) {
  ...
  if (sslOpts.secureOptions) {
    ssl.secureOptions =
      typeof sslOpts.secureOptions === 'undefined' || sslOpts.secureOptions === null
        ? constants.SSL_OP_NO_SSLv2 | constants.SSL_OP_NO_SSLv3
        : sslOpts.secureOptions;
  }
...
ReverseProxy.prototype.updateCertificates = async function (
  ...
  const siteCallback = (err, site) => {
    if (err) {
      if (err.message && err.message.contains('ENOENT: no such file or directory')) {
        this.log && this.log.warn(err, err.message);
      } else {
        this.log && this.log.error(err, "Error getting Let's Encrypt certificates");
        return;
      }
    }
...
ReverseProxy.prototype._getTarget = function (src, req, res) {
  ...
  return this.resolve(src, url, req).then((route) => {
    ...
    this.log && this.log.debug(`Proxying ${src + url} to ${path.posix.join(target.host, req.url)}`);
...
function createCredentialContext(key, cert, ca) {
  ...
  opts.secureOptions = constants.SSL_OP_NO_SSLv2 | constants.SSL_OP_NO_SSLv3;
