...
async function getCertificates(
  ...
  if (!le) {
    ...
    logger && logger.debug(`Creating the le instance... (${createOptsHash})`);
  }

  const onSite = (site) => {
    site.pems.fullchain = site.pems.cert + '\n' + site.pems.chain + '\n';
    logger && logger.info("Get Let's Encrypt certificates for " + domain);
  ...
  const onAdd = async (err, event) => {
    ...
    logger && logger.debug(`Fetched ${domain} cert.`);
  ...
  if (site) {
    logger && logger.debug(`Found the ${domain} cert.`);
    return onSite(site);
  }
